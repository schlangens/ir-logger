...
const {
  requireSession,
  requireWorkspace,
} = require("../middleware/workspace-guard");
const audit = require("../services/audit");
const hash = (s) => crypto.createHash("sha256").update(s).digest("hex");
const user = (u) => ({ id: u.id, email: u.email, name: u.name });
router.post("/workspaces", requireSession, (req, res, next) => {
...
router.get("/workspaces", requireSession, (req, res) => {
  try {
...
router.get("/workspaces/:id", requireWorkspace({}), (req, res, next) => {
  try {
    ...
    const w = db
      .prepare(
        "SELECT id,name,is_demo,expires_at,created_at FROM workspaces WHERE id=?",
      )
...
router.post("/invites/:token/accept", requireSession, (req, res, next) => {
  try {
    const db = req.app.locals.db;
    const inv = db
    ...
    db.transaction(() => {
      ...
      ).run(req.user.id, inv.workspace_id, inv.role);
...
router.get(
  ...
  (req, res, next) => {
    try {
      const rows = req.app.locals.db
        .prepare(
          "SELECT id,name,created_at,last_used_at FROM api_tokens WHERE workspace_id=?",
        )
        .all(req.workspace.id)
        .map((r) => ({
          id: r.id,
