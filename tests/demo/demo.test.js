...
test('demo creation seeds the full scenario, evidence, and persisted session grant', async (t) => {
  ...
  assert.equal(JSON.parse(session.session_json).demoWorkspaceId, workspaceId);
  assert.deepEqual(
    resolveWorkspaceAccess(db, { session: JSON.parse(session.session_json) }, workspaceId),
...
test('demo creation rate limit and limiter storage failure fail closed', async (t) => {
  ...
  for (let i = 0; i < 3; i++) assert.equal((await request(app).post('/api/demo')).status, 201);
  assert.equal((await request(app).post('/api/demo')).status, 429);
  db.exec('DROP TABLE rate_limits');
  assert.equal((await request(app).post('/api/demo').set('X-Forwarded-For', '198.51.100.10')).status, 503);
});
...
test('demo global ceiling is exclusive and count failures fail closed', async (t) => {
  ...
  for (let i = 0; i < 24; i++)
    db.prepare('INSERT INTO workspaces (id, name, is_demo, expires_at) VALUES (?, ?, 1, ?)').run(`active-${i}`, 'Active demo', future);
  assert.equal((await request(app).post('/api/demo').set('X-Forwarded-For', '198.51.100.20')).status, 201);
  const failure = appFor(t);
  failure.db.exec('DROP TABLE workspaces');
  assert.equal((await request(failure.app).post('/api/demo').set('X-Forwarded-For', '198.51.100.21')).status, 503);
});
...
test('demo creation rejects a full active global ceiling', async (t) => {
  ...
  const future = new Date(Date.now() + 60 * 60 * 1000).toISOString();
  for (let i = 0; i < 25; i++)
    db.prepare('INSERT INTO workspaces (id, name, is_demo, expires_at) VALUES (?, ?, 1, ?)').run(`full-${i}`, 'Active demo', future);
  const response = await request(app).post('/api/demo').set('X-Forwarded-For', '198.51.100.22');
