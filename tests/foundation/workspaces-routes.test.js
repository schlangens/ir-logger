...
test('workspace create/list/get enforce membership and response shape', async (t) => {
  ...
  const created = await owner.post('/api/workspaces').send({ name: 'Workspace' });
...
test('owner invites and invited user accepts by raw token hash', async (t) => {
  ...
  const workspace = await owner.post('/api/workspaces').send({ name: 'Invites' });
  ...
  assert.equal((await invited.post(`/api/invites/${rawToken}/accept`)).status, 404);
  ...
  db.prepare(
    ...
    db.prepare('SELECT id FROM users WHERE email = ?').get('owner-invite@example.test').id,
...
test('non-owners cannot invite or manage tokens, and token lifecycle hides secrets', async (t) => {
  ...
  const id = (await owner.post('/api/workspaces').send({ name: 'Tokens' })).body.workspace.id;
  ...
  assert.equal(
    (await analyst.post(`/api/workspaces/${id}/tokens`).send({ name: 'blocked' })).status,
    403,
  );
  const created = await owner.post(`/api/workspaces/${id}/tokens`).send({ name: 'Desktop' });
  assert.equal(created.status, 201);
