...
test("audit chain verifies and detects tampered hash", (t) => {
  ...
  t.after(() => db.close());
  db.prepare(
    "INSERT INTO workspaces (id, name) VALUES ('a', 'A'), ('b', 'B')",
  ).run();
  appendThree(db, "a");
  audit.append(db, {
    workspaceId: "b",
  ...
  const middle = db
    .prepare(
      "SELECT id FROM audit_log WHERE workspace_id = 'a' ORDER BY rowid LIMIT 1 OFFSET 1",
    )
    .get();
...
test("audit verifier detects tampered prev_hash", (t) => {
  ...
  const row = db
    .prepare(
      "SELECT id FROM audit_log WHERE workspace_id = 'a' ORDER BY rowid LIMIT 1 OFFSET 2",
    )
    .get();
  // Test-only raw-SQL integrity fixture: corrupt the third row's prev_hash.
  db.exec(
    `UPDATE audit_log SET prev_hash = '${"f".repeat(64)}' WHERE id = '${row.id}'`,
  );
  assert.deepEqual(audit.verify(db, "a"), {
    valid: false,
    checked: 3,
