...
test("registration regenerates a genuine pre-existing session", async (t) => {
  ...
  assert.equal(
    db.prepare("SELECT 1 FROM sessions WHERE sid = ?").get(oldSid),
    undefined,
  );
  assert.equal(
    db
...
test("login regenerates and drops demoWorkspaceId from a crafted session", async (t) => {
  ...
  const stored = JSON.parse(
    db.prepare("SELECT session_json FROM sessions WHERE sid = ?").get(newSid)
      .session_json,
  );
...
test("login limiter storage failure returns 503", async (t) => {
  ...
  db.prepare = (sql) => {
    if (String(sql).includes("rate_limits"))
